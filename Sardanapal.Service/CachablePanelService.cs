    ...
    where TEditableVM : class, new()
{
    ...
    public override async Task<IResponse<TKey>> Add(TNewVM Model)
    {
        ...
        await result.FillAsync(async () =>
        {
            ...
            await _cacheRepository.AddAsync(entityModel);
    ...
    public override async Task<IResponse<TVM>> Get(TKey Id)
    {
        ...
        await result.FillAsync(async () =>
        {
            var fetchModel = await _cacheRepository.FetchByIdAsync(Id);

            if (fetchModel == null)
            {
                fetchModel = await _repository.FetchByIdAsync(Id);
            }
    ...
    public override async Task<IResponse<bool>> Edit(TKey Id, TEditableVM Model)
    {
        ...
        await result.FillAsync(async () =>
        {
            ...
            await _cacheRepository.UpdateAsync(Id, editedModel);
